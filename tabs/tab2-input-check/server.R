tab2Server <- function(input, output, session, static_mix_maps) {
  # moduleServer(id, function(input, output, session, static_mix_maps) {
    ns <- session$ns
    ...
    # Calculate coverage summary for a specific intervention
    get_coverage_summary <- function(intervention, year_filter) {
      req(input$plan_select)
      
      # Get coverage data for the specified intervention
      ...
      # Calculate states with full, partial, and no coverage
...
output$intervention_tabs <- renderUI({
  ...
  intervention_panels <- lapply(interventions(), function(intervention) {
    ...
    nav_panel(
      ...
      layout_column_wrap(
        width = 1/3,
        # Full coverage value box
        value_box(
          title = "States with Full Coverage",
          value = summary_list$full_count,
        ...
        # Partial coverage value box
        value_box(
          title = "States with Partial Coverage",
          value = summary_list$partial_count,
          showcase = bsicons::bs_icon("dash-circle-fill"),
          theme = "warning",
          height = "100%",
          full_screen = FALSE
        ),
        
        # No coverage value box
        value_box(
          title = "States with No Coverage",
          value = summary_list$none_count,
      ...
      # Data table for this intervention
...
observe({
  req(interventions(), input$plan_select)
    ...
    local({
      ...
      output[[output_id]] <- renderDT({
        ...
        datatable(
          ...
          options = list(
            pageLength = 10,
            dom = 'lfrtip',  # Removed 'B' to disable export buttons
          ...
          rownames = FALSE,
          filter = 'top',  # This adds filter controls at the top of each column
